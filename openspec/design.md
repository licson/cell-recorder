...
- **Export integration**: Cell CSV export triggers a follow-up speedtest CSV export via `ExportSpeedTestUseCase.exportCsv()`. After the user saves the cell CSV, a second SAF document picker opens for `session_name_speedtest.csv`. The speedtest export columns match the spec: timestamp, download_bps, upload_bps, server_name, server_host, server_location, succeeded, error_message, data_sim_slot, rat_at_test, rsrp_at_test, band_at_test, network_type

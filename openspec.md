...
- **Foreign key constraints**: Enabled via `PRAGMA foreign_keys = ON` in `AppDatabase.CALLBACK.onOpen()`. All FK declarations (`CellRecordEntity.sessionId → sessions.id`, `CellRecordCaBandEntity.cellRecordId → cell_records.id`) with `ON DELETE CASCADE` are enforced at runtime. `deleteById` annotated `@Transaction` for atomicity. Batch inserts (`insertAll`, `insertCaBands`) annotated `@Transaction` for atomicity.
...
- **Sensor fusion**: `SensorFusionCollector` uses `TYPE_GAME_ROTATION_VECTOR` (no magnetometer needed) for heading delta tracking and `TYPE_LINEAR_ACCELERATION` for speed adjustment during GPS loss. Yaw is negated to convert from counterclockwise-positive (Euler) to clockwise-positive (navigational) convention. Linear acceleration is transformed from device frame to world frame (ENU) via **manual 3×3 matrix multiplication** (not `Matrix.multiplyMV` which requires 4×4), projected onto the current heading, and integrated into a speed delta with exponential decay (τ=10s) and ±50% clamping. Fallback heading is 0 and speed delta is 0 when sensors are unavailable. GPS extrapolation uses `movePoint(lat, lon, bearing, distance)` Haversine calculation.
- **RecordingService idempotency**: `startRecording()` cancels existing jobs before creating new ones; `onStartCommand` guards against calling `startRecording()` when `recordingJob.isActive == true` to prevent duplicate recording coroutines. Session ID guard uses `> 0` instead of `!= -1L` to reject `@PrimaryKey(autoGenerate=true)` default of 0.
- **Notification content intent**: `openIntent` uses `FLAG_ACTIVITY_CLEAR_TOP | FLAG_ACTIVITY_SINGLE_TOP` and `MainActivity` uses `launchMode="singleTop"` to prevent duplicate Activity instances on notification tap.
- **Point recording resilience**: `PointRecorder.recordPoint()` wraps per-snapshot database operations in try-catch — a single failed insert (e.g. FK violation) skips that snapshot and continues, rather than stopping the entire recording.
